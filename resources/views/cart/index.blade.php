...
@section('content')
    ...
    <section class="user-cabinet beauty-wrapper">
                        ...
                        @if(Cart::count())
                          ...
                          <tr>
                              ...
                              <td>
                                  ...
                                  <form action="{{route('basket.update', ['id'=>$row->rowId])}}" method="post"
                                      ...
                                      <div class="input-group btn-block" style="max-width: 200px">
                                          ...
                                          <input type="text"
                                                 onchange="this.closest('form') && this.closest('form').submit()"
                                          ...
                                          <span class="input-group-btn">
                                                    <div style="display: flex;">                      
                                                        <button type="submit" data-toggle="tooltip" title=""
                                                                ...
                                                                data-original-title="Обновить"><i
                                                                    class="fa fa-redo"></i></button>
                                  </form>
                                  <form action="{{route('remove',['id'=>$row->rowId])}}" method="get">
                                      @csrf
                                      <button onclick="return confirm('are you sure?')" type="submit"
                                              class="btn btn-danger"><i class="fa fa-times-circle"></i></button>
                                  </form>
        ...
        <form action="{{route('checkout')}}" method="get">
            @csrf
            <input type="hidden" name="shop_id" hidden value="{{Auth::user()->get_shop_by_user_iD()}}">
            <div class="buttons clearfix">
                <div class="pull-left"><a href="/" class="btn btn-default"> Продолжить покупки </a></div>
                @if( !Session::has('alert') )
                    @if(Cart::count())
                        <div class="pull-right">
                            <button @if(Session::has('error')) disabled @endif type="submit" class="btn btn-primary">
                                Оформление заказа
                            </button>
                        </div>
                    @endif
    ...
    <footer>
        <div class="footer-bar"> ® copyright 2021. | Разработано в <a href="http://steepcoder.uz/site">steepcoder.uz</a>
        </div> <!-- /.footer-bar -->
    </footer>
